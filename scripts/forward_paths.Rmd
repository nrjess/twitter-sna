...
actor = c(986) # 986 is the integer ID for RepKrajewski

# extract the forward reachable paths from a node
RepKrajewski_fwd <- tPath(net1.dyn,
                          v = actor,
                          direction = "fwd",
                          start = 0,
                          end = max(es$terminus))
...
```{r fwd-plot1}
plot(RepKrajewski_fwd)

```

```{r fwd-plot2}
plot(transmissionTimeline(RepKrajewski_fwd,
                     jitter=TRUE,
...
Krajewski <- as.data.frame(RepKrajewski_fwd) %>%
  mutate(vertex.id = row_number()) %>%
  filter(tdist != 'Inf') %>%
  select(vertex.id, tdist)

# tail must be an element of Krajewski$vertex.id and
# the corresponding onset must be greater or equal to the tdist associated with that vertex.id
es.Krajewski <- es %>%
  right_join(Krajewski, by = c("tail" = "vertex.id")) %>%
  filter(onset>=tdist)

# filter nodes--keep only those in edgelist
vs2 <- vs %>%
  filter(vertex.id %in% es.Krajewski$tail | vertex.id %in% es.Krajewski$head) %>%
...
render.d3movie(net2.dyn,
               ...
               vertex.sides =  function(slice) {
                 case_when((slice %v% "username" == "RepKrajewski") ~ 3,   # RepKrajewski is a triangle
                           (slice %v% "username" != "RepKrajewski") ~ 50)  # everyone else is a circle
               ...
               vertex.tooltip = function(slice) {     # display node attributes when you click on an node
                 ...
                 #label.cex = .6,
                 #label.col = "blue",
               main = 'School choice ties: forward paths from RepKrajewski',
